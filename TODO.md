# WebWrap Studio: Product Hunt Launch Plan
...
3. **Config over code.** Every generated-app feature is a key in `webwrap.config.json`. New features add a key and a UI control, not a new template.
...
| T8 | Icon picker: upload an image, or auto-fetch the favicon (any image Electron can decode, normalised to a 1024px PNG) | P0 | In progress | T5 |
| T9 | Convert icon to `.ico` and `.icns` (electron-builder converts a 512px+ PNG itself) | P1 | Not needed | T8 |
| T10 | In-app build: run npm install and electron-builder with live log, cancel and 20-minute timeout | P0 | In progress | T5, T29 |
| T11 | "Show installer" button after build | P1 | In progress | T10 |
| T12 | Handle missing Node/npm (detect and show install guidance) | P0 | In progress | T10 |
| T13 | Live preview of the URL (webview) before generating | P1 | Not picked | T31 |
| T14 | Generated app: open external links in default browser | P1 | In progress | T29 |
| T15 | Generated app: single-instance lock | P1 | In progress | T29 |
| T16 | Generated app: offline / error page with retry | P1 | Not picked | T29 |
| T17 | Generated app: window config (size, frameless, always-on-top, remember position) | P2 | Not picked | T29, D15 |
| T18 | Generated app: tray icon, minimize to tray, start on login | P2 | Not picked | T29, D15 |
| T19 | Generated app: inject custom CSS and JS per site | P2 | Not picked | T29, D15 |
| T20 | Generated app: permission prompts and user-agent override | P2 | Not picked | T29, D15 |
| T21 | Project history and re-open or regenerate | P2 | Not picked | T29 |
| T22 | One-click presets, stored as config JSON files | P1 | Not picked | T29, T8 |
...
| T26 | Crash and error reporting (opt-in, no tracking by default) | P2 | Not picked | T10 |
| T27 | Replace placeholder metadata in `package.json`: homepage, repository and bugs are set. Author and license still need your input | P0 | In progress | none |
| T28 | Tagged release build with signed installers attached to GitHub Releases | P0 | Not picked | T4, T10, T23, T27 |
| T29 | Config-driven generator: `webwrap.config.json`, the generated `main.js` reads it | P0 | Done | T2 |
...
| T32 | Default output folder (Documents/WebWrap Studio) with a "Change" link | P1 | Done | T1 |
| T33 | Update the Electron pin in `package.json` (`^32.0.0`) to match what is installed (38.8.6), then re-run the app and a real build | P1 | Not picked | none |
...
| D2 | Replace placeholder app icon with real brand icon (1024x1024) | P0 | Not picked | D3 |
| D3 | Brand kit: name check, logo, colors, typography | P0 | Not picked | M1 |
| D4 | Build-progress screen and success state with illustrations | P0 | In progress | T10 |
| D5 | Inline validation and error states in the form | P1 | Done | D1 |
| D6 | Icon picker UI with preview | P1 | In progress | T8 |
| D7 | Preview pane layout | P1 | Not picked | T13 |
| D8 | Preset gallery UI | P1 | Not picked | T22 |
| D9 | Empty, loading and error states with illustrations | P1 | In progress | D1 |
| D10 | Accessibility pass (focus order, contrast, labels, keyboard): done by the UI agent, contrast estimated, not measured | P1 | In progress | D4, D5, D6 |
| D11 | Dark and light theme | P2 | Not picked | D1 |
| D12 | Onboarding tour or sample project | P2 | Not picked | D9 |
...
| D15 | Single collapsed "Advanced" panel, the home for future options | P2 | In progress | D13, T29 |
| D16 | Shared design tokens (`tokens.css`) and app restyle | P1 | Done | D3 |
| D17 | Fix the folder line when no folder is known ("Saves to no folder chosen") | P2 | Not picked | D13 |
...
| W7 | Download section with OS detection and GitHub Releases links (now point to Abhijat-Abhijat/Web-Wrap-Studio) | P0 | Done | T28 |
...
| W13 | Lighthouse pass. Layout checked at 375px (no horizontal scroll). Lighthouse itself not run: no Chrome on this machine | P1 | In progress | W10 |
| W14 | Connect domain and HTTPS, deploy. Replace `webwrap.example` in sitemap, robots.txt, canonical and og tags, and the footer email | P0 | Not picked | M6, W10 |
...
| M5 | License decision. Draft recommends MIT. Owner decision | P0 | In progress | M4 |
| M6 | Buy the domain | P0 | Not picked | M3, D3 |
| M7 | Website copy. Written into the site, needs owner read-through | P0 | In progress | M3, W1 |
| M8 | Demo GIF or video. Shot list and script written in `07-demo-script.md`, not recorded | P0 | In progress | T10, D4, D13 |
| M9 | Product Hunt gallery: 3 to 5 screenshots plus thumbnail | P0 | Not picked | D3, D4, M8 |
...
Next: T33 (Electron version pin), then run the app for real and do one real build. Nothing added in the last two waves has run under Electron yet.
...
- Author name for `package.json`. Legal entity, contact and jurisdiction for the terms.
- Launch date. Domain name.

...
- Electron Forge and electron-builder (the route WebWrap Studio generates configs for)
...
| Websktop | Windows .exe; macOS .dmg/.app; Linux .AppImage (vendor pages) | Says "wrapper architecture", not Electron or Tauri; Windows build uses WebView2 | Windows installer about 4 MB (vendor claim) | Free tier with 0 apps and 0 builds; $10 per app and $2 per build beyond that (vendor page); a search summary lists paid plans from $12 per month, which conflicts | Unknown | No, browser dashboard | [home](https://websktop.net/), [URL to EXE](https://websktop.net/url-to-exe), [website-to-desktop-app page](https://websktop.net/website-to-desktop-app) (a search summary of this page listed plans from $12 per month; not confirmed, price UNVERIFIED) |

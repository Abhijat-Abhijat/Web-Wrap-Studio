...
async function generateProject({ url, name, description, outputDir, iconDataUrl, options }) {
  ...
  const pkg = {
    ...
    build: {
      ...
      files: ["main.js", "preload.js", "index.html", "offline.html", "webwrap.config.json", "build/icon.png"],
  ...
  await fs.writeFile(
    path.join(projectDir, "webwrap.config.json"),
...
function wrappedMainTemplate() {
  ...
  // All user data comes from webwrap.config.json, never interpolated here.
  return `const { app, BrowserWindow, shell, Tray, Menu, nativeImage, screen, ipcMain } = require("electron");
const fs = require("fs");
const path = require("path");
const config = require("./webwrap.config.json");
...
ipcMain.on("webwrap:retry", (event) => {
...
contextBridge.exposeInMainWorld("webwrap", { retry: () => ipcRenderer.send("webwrap:retry") });
`;
}
...
<script>
  const retry = () => window.webwrap && window.webwrap.retry();
